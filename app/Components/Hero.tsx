...
const highlightPills = ["GSAP Motion", "Smooth Lenis", "TypeScript"] as const;
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col items-center px-6 py-24 text-center md:px-8 lg:px-12">
        <div className="w-full">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/60 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.25em] text-black/70">
            <Sparkles className="h-3 w-3 text-primary" aria-hidden="true" />
            v1.0 • Ultimate Starter Kit
          </div>

          <h1 className="mb-6 text-4xl font-semibold tracking-tight text-black md:text-6xl lg:text-7xl">
            Build Digital <br />
          ...
          <p className="mx-auto mb-10 max-w-3xl text-md text-black/70 md:text-xl font-medium">
            The definitive foundation for high-end web experiences. Engineered
            with Next.js, TypeScript, GSAP, and Lenis to provide buttery-smooth
            motion and unmatched performance right out of the box.
          </p>

          <div className="mb-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <button className="group flex items-center justify-center gap-2 rounded-full bg-primary px-8 py-4 text-base font-bold uppercase tracking-[0.2em] text-white transition-opacity hover:opacity-90">
              Clone Kit
              <ArrowRight
            ...
            <button className="flex items-center justify-center rounded-full border border-black/10 bg-white/60 px-8 py-4 text-base font-bold uppercase tracking-[0.2em] text-black/80 backdrop-blur transition-all hover:bg-white/70">
              Documentation
            </button>
          </div>

          <ul className="mb-12 flex flex-wrap items-center justify-center gap-3 text-xs uppercase tracking-[0.2em] text-black/70">
            {highlightPills.map((pill) => (
              <li
                key={pill}
                className="rounded-full border border-black/10 bg-white/60 px-4 py-2 backdrop-blur font-semibold"
              >
                {pill}
              </li>
            ))}
          ...
          <div className="grid gap-4 rounded-2xl border border-black/10 bg-white/60 p-6 backdrop-blur-sm sm:grid-cols-3">
            {heroStats.map((stat) => (
              <div key={stat.label} className="space-y-1">
                <div className="text-xs uppercase tracking-[0.3em] text-black/50 font-bold">
                  {stat.label}
                </div>
                <div className="text-3xl font-bold text-black italic">
                  {stat.value}
                </div>
              </div>
            ))}
